	...
	<body class="is-preload">
		...
		<!-- Page Wrapper -->
			<div id="page-wrapper">
				...
				<!-- Header -->
					<header id="header">
						<h1><a href="./ingreso_d.php" class="special after">Regresar</a></h1>
						<nav>
							<a href="#menu">Menu</a>
						</nav>
					</header>
...
$CodigoD=$_POST["CodigoD"]; // instruccion para obtener parametros que se llenaron desde el formulario
$NombreD=$_POST["NombreD"];

$dbconn = pg_connect("host=localhost dbname=ProyectoCCV user=postgres password=12345")
    or die('No se ha podido conectar: ' . pg_last_error());
    $query2 = "SELECT * FROM Departamento WHERE CodigoD=$CodigoD";
    $reul = pg_query($query2);
   
    if (pg_num_rows($reul) > 0){
?>
        <p>El departamento con  código <?php echo $CodigoD ?> ya existe</p>
        <?php
        pg_free_result($reul);
        ...
        ?>
        <p>¡El Departamento fue insertado exitosamente! </p>
        <?php
