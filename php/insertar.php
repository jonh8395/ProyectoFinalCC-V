	...
	<body class="is-preload">
		...
		<!-- Page Wrapper -->
			<div id="page-wrapper">
				...
				<!-- Header -->
					<header id="header">
						<h1><a href="./ingreso.php" class="special after">Regresar</a></h1>
						<nav>
							<a href="#menu">Menu</a>
						</nav>
					</header>
...
$querrr = "SET timezone TO 'America/Guatemala'";
$ros = pg_query($querrr);
...
<?php 
	...
	if($tipo == "1"){
		$query5 = "SELECT * FROM Registro WHERE (Fecha = current_date) AND (TipoMarca = 'E') AND (CodigoE = $CodigoE) AND (Hora is not null)";
		$r5 = pg_query($query5);

		$query7 = "SELECT * FROM Registro WHERE (Fecha = current_date) AND (TipoMarca = 'S') AND (CodigoE = $CodigoE) ";
		$r7 = pg_query($query7);
    ...
    $query1 = "SET timezone TO 'America/Guatemala'";
    $res = pg_query($query1);

    $query2 = "INSERT INTO Registro VALUES($CodigoE,current_date,'E',current_time)";
    $reul = pg_query($query2);
   
    $query3 = "SELECT NombreE FROM Empleado WHERE CodigoE = $CodigoE";
    $r = pg_query($query3);
    $NombreE ="";
    while($line = pg_fetch_assoc($r)){
...
pg_close($dbconn);
			...
			$query6 = "SELECT * FROM Registro WHERE (Fecha = current_date) AND (TipoMarca = 'S') AND (CodigoE = $CodigoE)";
			$r6 = pg_query($query6);
			$queryn = "SELECT * FROM Registro WHERE (Fecha = current_date) AND (TipoMarca = 'E') AND (CodigoE = $CodigoE)";
			$rr7 = pg_query($queryn);
			  if(pg_num_rows($r6) > 0){
				...
				$query1 = "SET timezone TO 'America/Guatemala'";
				$res = pg_query($query1); 
				$quer = "INSERT INTO Registro VALUES($CodigoE,current_date,'E',NULL)";
				$rrw = pg_query($quer);
				$query2 = "INSERT INTO Registro VALUES($CodigoE,current_date,'S',current_time)";
		$reul = pg_query($query2);
