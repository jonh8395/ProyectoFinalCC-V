	...
	<body class="is-preload">
		...
		<!-- Page Wrapper -->
			<div id="page-wrapper">
				...
				<!-- Header -->
					<header id="header">
						<h1><a href="./ingreso_u.php" class="special after">Regresar</a></h1>
						<nav>
							<a href="#menu">Menu</a>
						</nav>
					</header>
								...
								<li>
									<div class="dropdown3">
										...
										<div class="dropdown3-content-cuatro">
											...
											<a href="./ingreso_usuario.php">Consultar</a>
										</div>
									</div>
								</li>
								<br><br><br>
...
?>
        <p>El usuario con  username <?php echo $Username ?> ya existe</p>
        <?php
        pg_free_result($reul);

    }else{

        $query = "INSERT INTO Usuario Values ('$Username','$ClaveU')";

        $result = pg_query($query);
        pg_query("CREATE USER $Username WITH PASSWORD '$ClaveU'") or die("No ha sido posible registrar al usuario");
        pg_query("GRANT SELECT,INSERT,DELETE,UPDATE ON Permiso TO $Username") or die("No ha sido posible otorgar permisos al usuario");
        pg_query("GRANT SELECT ON Empleado TO $Username");
        ?>
        <p>¡El Usuario fue insertado exitosamente! </p>
        <?php
